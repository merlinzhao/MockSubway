...
export class TripRouteController {
    ...
    async routeTrip(request: Request, response: Response, next: NextFunction) {
        ...
        if (typeof origin !== 'string' || origin.trim().length === 0) response.status(400).json({ error: 'Invalid origin parameter' });
        if (typeof destination !== 'string' || destination.trim().length === 0) response.status(400).json({ error: 'Invalid destination parameter' });
        ...
        try {
            ...
            response.status(200).json({ route: path });
        } catch (error) {
